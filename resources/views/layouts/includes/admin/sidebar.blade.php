      ...
      <ul class="space-y-2 font-medium">
         ...
         @else
            ...
            <button type="button" class="flex items-center w-full justify-between px-2 py-1.5 text-body rounded-base hover:bg-neutral-tertiary hover:text-fg-brand group" aria-controls="dropdown-example" data-collapse-toggle="dropdown-example">
                  <span class ="w-6 h-6 inline-flex justify-center text-gray-500">
                    <i class="{{ $link['icon'] }}"></i>
                </span>
            ...
            @foreach ($link['submenu'] as $item)
               <li>
                     <a href="{{ $item['href'] }}" class="pl-10 flex items-center px-2 py-1.5 text-body rounded-base hover:bg-neutral-tertiary hover:text-fg-brand group {{ $item['active'] ? 'bg-gray-200' : '' }}">{{ $item['name'] }}</a>
                  </li>
            @endforeach
